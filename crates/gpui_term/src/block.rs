        ...
        match event {
            Osc133Event::PromptStart => {
                ...
                } = self.state
                {
                    ...
                    let output = read_output(term, output_start_line, history_at_start, cursor_line);
...
fn read_command_at_cursor(term: &Term<ZedListener>, cursor_line: Line, cursor_col: Column) -> String {
...
mod tests {
    ...
    fn test_is_prompt() {
        ...
        assert!(is_prompt("% ", Column(2)));
        assert!(!is_prompt("hello world this is a very long output line", Column(43)));
